...
- **Still TODO (opts #2–#5)** — per-criterion sub-score feedback; section-level refine (regenerate only the weakest section); cheap-evals drive refine + Sonnet panel only as final gate; cache the static résumé/JD blocks in the human messages. These touch the scoring loop — do carefully + measure via telemetry.
