...
class AnthropicEvaluatorAgent(BaseEvaluatorAgent):
    ...
    async def run(self, resume_json: dict, job_description: str, profession_config: dict, source_resume_text: str = "") -> dict:
        try:
            messages = await anthropic_evaluator_messages(resume_json, job_description, profession_config, source_resume_text)
            response = await self._model().ainvoke(messages)
            record_usage(settings.anthropic_evaluator_model, self.name, response)
