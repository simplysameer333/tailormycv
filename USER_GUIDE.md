...
> **AI-powered resume builder** that tailors your CV to every job posting using multiple AI models.
...
After registering, complete your **Profile** (`/profile`) with:
- Full name, contact details, LinkedIn URL
- Your target roles and primary skill
- Upload your existing resume to pre-fill everything automatically

This profile is reused across all your builder sessions — you only fill it in once.
...
Upload your current resume as a **PDF or DOCX** (max 5 MB).

**If you have saved resumes in your Library (Plus/Pro)**, they appear here — click **"Use this resume"** or **"Tailor with this"** to skip re-uploading.

**Tailoring from Find Jobs?** The job title and employer appear at the top of every step so you always know which role you're building for. Click the banner to re-open the original job listing.
...
- **Preferred tone** — Professional, Conversational, or Technical
- **Key skills** — your core competencies (pre-filled from your profile)
- **Additional notes** — any instructions to the AI (e.g. *"emphasise leadership experience"*)

> **Tip:** If anything looks wrong, correct it here — these details appear verbatim on the output.
...
**Template** — pick from the available designs. The preview image shows what the output will look like.

**Output format** — DOCX only (Free) or DOCX + PDF (Plus/Pro).

**Additional instructions** *(optional)* — last-minute AI direction, e.g. *"use UK English"* or *"keep to one page"*.
...
3. AI Evaluators score it (1 evaluator on Free, 2 on Plus, 3 on Pro)
...
- **"Bold key skills" checkbox** — when checked (default), matched skills are highlighted bold in the exported file to stand out to recruiters
- **Regenerate the whole resume** — click the main Regenerate button with optional guidance notes
- **Regenerate a single section** *(Pro only)* — each section (Summary, Experience, Education, etc.) has its own Regenerate button with a feedback field

**Quality badge** — shown after generation:
...
**Locked Facts** *(Pro only)* — pin specific facts (e.g. *"Company: Google"*, *"Degree: BSc Computer Science"*) that the AI must never change. Useful when you have exact details that must appear verbatim.

---

### Step 6 — Download

Click **Generate Files** to produce your DOCX and PDF.
...
| **Use in builder** | Start a new session using this resume (Step 1 → "Use this resume") |
| **Tailor for a job** | Start a new session with this resume pre-loaded, ready for a specific JD |
...
If no new jobs were found that day, you receive a brief no-results notification.

### Managing alerts

- **Toggle** the switch to pause/resume an alert without deleting it
- **Edit** to change search terms or name
- **Delete** to remove permanently
- **Send test** to send a sample email immediately (useful to verify setup)

> **Note:** Alerts are automatically deactivated if your account is downgraded from Plus/Pro to Free. Re-subscribe to reactivate them.

---

## 7. Profile & Account

Go to **Sameer Hameed** (top-right) → **My Profile** to manage:

### Personal details

- Full name, email, phone, LinkedIn URL, location
- Upload your resume to auto-fill all fields using AI extraction
- **Target roles** — list of roles you're interested in (used to pre-fill job search)
- **Primary skill** — your core technical or professional skill (e.g. *Python*, *Product Management*)

### Changing your password

Available in the Profile page for email/password accounts. Google OAuth accounts manage passwords through Google.
...
- **Review the output before downloading** — edit inline on the Preview page to correct anything that doesn't look right.
- **Use the quality badge as a guide** — if it says "Reviewed", try regenerating with feedback notes on the sections that need work.
...
- Upload your most comprehensive existing resume — the AI uses it as a base and restructures it for each role.
- DOCX output is fully editable in Microsoft Word or Google Docs if you want to make final tweaks.
