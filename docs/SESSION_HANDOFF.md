...
### 3. Per-user daily budgets (account-level guardrail)
- **`services/usage_service.py`** — per-(user, UTC day) `daily_usage` collection (TTL-indexed, 7d). `check_daily_budget()` (429 before any LLM cost), `increment_daily_usage()` after each run (full + section-regen paths). Anonymous users fall back to the per-session cap.
- **Limits are Mongo tier config** — `daily_ai_calls` + `daily_cost_cents` added to `DEFAULT_LIMITS`/`LIMIT_LABELS`. `load_config` now **backfills** missing keys into an existing `tier_config` doc (else `get_limit` would return 0 = blocked) and persists them. Admin tier editor renders them generically; `PricingTiers` shows the daily call budget per plan; `config.ts TIER_LIMITS` kept in sync.
